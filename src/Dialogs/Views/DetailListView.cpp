...
DetailListView::DetailListView(wxWindow *parent, wxWindowID id, const wxString &title, const wxPoint &pos, const wxSize &size, long style) : wxFrame(parent, id, title, pos, size, style)
{
	...
	m_DatePickerFrom = new wxDatePickerCtrl(mainPane, wxID_ANY, wxDefaultDateTime, wxDefaultPosition, wxDefaultSize, wxDP_DEFAULT);
	tPickerSizer->Add(m_DatePickerFrom, 0, wxALL, 5);

	m_DatePickerTo = new wxDatePickerCtrl(mainPane, wxID_ANY, wxDefaultDateTime, wxDefaultPosition, wxDefaultSize, wxDP_DEFAULT);
	tPickerSizer->Add(m_DatePickerTo, 0, wxALL, 5);

	tMainSizer->Add(tPickerSizer, 0, wxALL, 5);

	m_ListView = new wxListCtrl(mainPane, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLC_REPORT | wxLC_SORT_ASCENDING | wxLC_SINGLE_SEL);
	tMainSizer->Add(m_ListView, 1, wxEXPAND, 5);

	m_buttonReuseData = new wxButton(mainPane, wxID_ANY, wxT("Reuse work item"), wxDefaultPosition, wxDefaultSize, 0);
	tMainSizer->Add(m_buttonReuseData, 0, wxALL, 5);

	mainPane->SetSizer(tMainSizer);
	mainPane->Layout();

	//this->SetClientSize(mainPane->GetBestSize());
